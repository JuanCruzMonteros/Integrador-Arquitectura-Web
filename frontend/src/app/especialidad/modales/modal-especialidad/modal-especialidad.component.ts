  ...
  crearModificarEspecialidad() {
    ...
    if (this.datosEspecialidad == null) {
      ...
      this._especialidadServicio.crear(especialidad).subscribe({
        next: (data) => {
          if (data.isExitoso) {
            this._sharedService.mostrarAlerta('La Especialidad ha sido grabada con Exito!',
              'Completo');
            this.modal.close("true");
          }
          else
            this._sharedService.mostrarAlerta('No se pudo crear la especialidad',
              'Error!');
        },
        error: (e) => {
          this._sharedService.mostrarAlerta(e.error.mensaje,
            'Error!');
        }
      })
    }
      ...
      this._especialidadServicio.editar(especialidad).subscribe({
        next: (data) => {
          if (data.isExitoso) {
            this._sharedService.mostrarAlerta('La Especialidad ha sido actualizada con Exito!',
              'Completo');
            this.modal.close("true");
          }
          else
            this._sharedService.mostrarAlerta('No se pudo actualizar la especialidad',
              'Error!');
        },
        error: (e) => {
          this._sharedService.mostrarAlerta(e.error.mensaje,
